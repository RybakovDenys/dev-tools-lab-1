...
def bounding_box(cells: set[tuple[int, int]]) -> tuple[int, int, int, int] | None:
    ...
    @details Iterates once over coordinates collecting min/max extents.
    @note Returns None for an empty input set to distinguish from a degenerate box.
    @example Basic usage
        box = bounding_box({(2,1),(4,3),(3,2)})  # (2,1,4,3)
    @example Empty set
...
def coords(*pairs):
    ...
    @details Accepts any number of (x,y) tuples and returns them as a set; duplicates collapse.
    @example
        live = coords((1,1), (2,2), (1,1))  # {(1,1), (2,2)}
    """
    return set(pairs)
...
def translate(cells: set[tuple[int, int]], dx: int, dy: int, width: int | None = None, height: int | None = None) -> set[tuple[int, int]]:
    ...
    @example Absolute translation
        translate({(1,1),(2,2)}, 3, -1)  # {(4,0),(5,1)}
    @example Toroidal translation
        translate({(4,4)}, 1, 1, width=5, height=5)  # {(0,0)}
    """
...
def canonicalize(cells: set[tuple[int, int]]) -> set[tuple[int, int]]:
    ...
    @details Useful for comparing pattern shapes independent of translation. Empty input yields empty set.
    @example Basic normalization
        canonicalize({(2,3),(3,3),(2,4)})  # {(0,0),(1,0),(0,1)}
    @example Idempotence
        s = {(0,0),(1,0)}; assert canonicalize(s) == s
    """
    if not cells:
        return set()
...
class TestGameOfLife(unittest.TestCase):
    ...
    def test_underpopulation(self):
        """!
        @brief A live cell (5, 5) with 1 neighbor should die.
        """
    ...
    def test_survival_2_neighbors(self):
        """!
        @brief A live cell (5, 5) with 2 neighbors should survive.
        """
    ...
    def test_survival_3_neighbors(self):
        """!
        @brief A live cell (5, 5) with 3 neighbors should survive.
        """
    ...
    def test_overpopulation(self):
        """!
        @brief A live cell (5, 5) with 4 neighbors should die.
        """
    ...
    def test_reproduction(self):
        """!
        @brief A dead cell (5, 5) with 3 neighbors should become alive.
        """
    ...
    def test_stasis_dead_cell(self):
        """!
        @brief A dead cell (5, 5) with 2 or 4 neighbors should remain dead.
        """
    ...
    def test_still_life_block(self):
        """!
        @brief The 'Block' pattern should remain stable.
    ...
    def test_still_life_beehive(self):
        """!
        @brief The 'Beehive' pattern should remain stable.
    ...
    def test_still_life_boat(self):
        """!
        @brief The 'Boat' pattern should remain stable.
    ...
    def test_oscillator_blinker_period_1(self):
        """!
        @brief The 'Blinker' should change orientation after 1 step.
    ...
    def test_oscillator_blinker_period_2(self):
        """!
        @brief The 'Blinker' should return to its initial state after 2 steps.
        """
        blinker_h = coords((1, 2), (2, 2), (3, 2))
        self.game.set_state(blinker_h)
        self.game.step()
    ...
    def test_oscillator_toad_period_1(self):
        """!
        @brief The 'Toad' should transition to phase 2 after 1 step.
    ...
    def test_oscillator_toad_period_2(self):
        """!
        @brief The 'Toad' returns to its initial state after 2 steps (period-2).
        """
        toad_p1 = coords((2, 1), (3, 1), (4, 1), (1, 2), (2, 2), (3, 2))
        self.game.set_state(toad_p1)
        self.game.step()
    ...
    def test_torus_reproduction_top_left_corner(self):
        """!
        @brief A cell (0, 0) should be born from neighbors wrapping around the edges.
        """
    ...
    def test_torus_overpopulation_right_edge(self):
        """!
        @brief A cell (9, 5) on the right edge should die from 4 neighbors wrapping around.
        """
    ...
    def test_torus_survival_bottom_edge(self):
        """!
        @brief A cell (5, 9) on the bottom edge should survive with 2 neighbors wrapping around.
        """
    ...
    def test_torus_blinker_on_edge(self):
        """!
        @brief A 'Blinker' crossing the edge should oscillate correctly.
        """
    ...
    def test_spaceship_glider_move(self):
        """!
        @brief The 'Glider' should move diagonally after 4 steps.
    ...
    def test_empty_board(self):
        """!
        @brief An empty grid should remain empty.
        """
    ...
    def test_full_wrapping_board_dies(self):
        """!
        @brief With wrap=True (toroidal), a fully filled grid becomes empty after 1 step.
        """
    ...
    def test_full_board_bounded_differs(self):
        """!
        @brief With wrap=False (bounded), a full 3x3 does not die completely: corners survive.
        """
    ...
    def test_r_pentomino_chaos(self):
        """!
        @brief The 'R-pentomino' should evolve across first two steps without relying on exact coordinates.
    ...
    def test_get_state_is_defensive_copy(self):
        """!
        @brief get_state() should return a copy, not a reference, to the internal state.
        """
    ...
    def test_constructor_validation(self):
        """!
        @brief Constructor validates dimensions and wrap type.
        """
    ...
    def test_bounding_box_helper(self):
        """!
        @brief The bounding_box helper should work correctly.
        """
    ...
    def test_set_state_wraps_out_of_bounds(self):
        """!
        @brief set_state should normalize out-of-bounds coordinates when wrap=True.
        """
    ...
    def test_set_state_ignores_out_of_bounds(self):
        """!
        @brief set_state should ignore out-of-bounds coordinates when wrap=False.
        """
    ...
    def test_set_state_rejects_invalid_types(self):
        """!
        @brief set_state should raise ValueError on invalid entries (non 2-int tuples) with clear cases.
        """
        g = GameOfLife(5, 5, wrap=True)
        invalid_inputs = [
